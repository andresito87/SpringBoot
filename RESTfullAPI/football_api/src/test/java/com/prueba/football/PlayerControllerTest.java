...
public class PlayerControllerTest {
        ...
        public void testListPlayers() throws Exception {
                ...
                // Act: Perform a GET request to the endpoint /players
                MvcResult result = mvc
                                .perform(MockMvcRequestBuilders.get("/players").accept(MediaType.APPLICATION_JSON))
                                .andExpect(status().isOk())
                                .andExpect(MockMvcResultMatchers.jsonPath("$", hasSize(2)))
                                .andReturn();

                String json = result.getResponse().getContentAsString();
                ObjectMapper mapper = new ObjectMapper(); // ObjectMapper is from Jackson library, to convert objects to
                                                          // JSON and vice versa
                mapper.registerModule(new JavaTimeModule()); // Jackson module to handle Java 8 date/time types
                List<Player> returnedPlayers = mapper.readValue(json,
                                mapper.getTypeFactory().constructCollectionType(List.class, Player.class));

                // Assert: Check that the response contains the expected players
        ...
        @Test
        public void testCreatePlayer_doesnt_exists() throws Exception {
                Player player = new Player("12345678", 5, "Ivana ANDRES", "Defender", LocalDate.of(1994, 7, 13));
                ...
                String playerJsonResult = result.getResponse().getContentAsString();

                Player returnedPlayer = mapper.readValue(playerJsonResult, Player.class);
